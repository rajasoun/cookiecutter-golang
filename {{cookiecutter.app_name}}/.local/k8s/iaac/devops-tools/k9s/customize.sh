...
function setup(){
    ...
    if [ ! -f "$MAC_K9S_HOME/plugin.yml" ]; then
        cp $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/devops-tools/k9s/plugin.yml $MAC_K9S_HOME
...
function test(){
    ...
    find  $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/devops-tools/k9s -type f -name '*.yml' -print0 | while IFS= read -r -d $'\0' file;
    do
