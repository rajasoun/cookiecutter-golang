...
function create_cluster(){
    try k3d cluster create --registry-config <(cat $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/kubernetes/k3d/config/registries.yaml | envsubst) --config  <(cat $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/kubernetes/k3d/config/k3d-config.yaml | envsubst)
...
function test(){
    ...
    $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/test/validate.sh || result=1
    return $result
}
