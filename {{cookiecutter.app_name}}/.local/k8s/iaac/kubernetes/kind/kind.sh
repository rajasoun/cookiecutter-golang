...
function create_cluster(){
    try kind create cluster --name $CLUSTER_NAME  --wait 5m --config="$GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/kubernetes/kind/config/kind.yaml" 
...
function test(){
    ...
    if [[ $(is_kind_cluster_exists) == "false" ]]; then
        ...
        try $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/test/validate.sh || result=1
    fi
    return $result
}
