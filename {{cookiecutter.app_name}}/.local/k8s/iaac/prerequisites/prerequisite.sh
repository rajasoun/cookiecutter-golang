...
function setup(){
    ...
    brew bundle --file $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/Brewfile
    shift 1 & do_audit $@
...
function list_installed_devops_tools(){
    local tools=($(brew bundle --file $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/Brewfile list))
...
function teardown(){
    ...
    brew bundle --file $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/global/Brewfile --cleanup
    brew cleanup
    rm -fr bin/*
    shift 1 & do_audit $@
