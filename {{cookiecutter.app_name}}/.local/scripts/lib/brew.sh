...
function check_brew_packages() {
    ...
    PACKAGE_LIST=($(grep -v "^#\|^$" $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/Brewfile | awk '{print $2}' |  tr -d '"'))
    LABEL=$1
    echo -e "\n🧪 Testing $LABEL"
...
function check_devops_tools(){
    ...
    MERGED_FILE_CONTENT="$(cat $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/global/Brewfile)\n$(cat $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/Brewfile)"
