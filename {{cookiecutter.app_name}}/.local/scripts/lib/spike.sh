...
function brew_install(){
    local app="$1"
    local directory="$GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local"
...
function brew_uninstall(){
    ...
    local brewfile="$GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/$app/Brewfile"
    if [ -f "$brewfile" ]; then
        pretty_print "${BLUE}Uninstalling Brewfile for $app ${NC}\n"
        pretty_print "Brewfile -> $brewfile\n"
        rm -fr /tmp/Brewfile
        cat  $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/global/Brewfile > /tmp/Brewfile
        echo -e "\n" >> /tmp/Brewfile
        cat $GIT_BASE_PATH/$IAAC_LIB_DIR/iaac/prerequisites/local/Brewfile >> /tmp/Brewfile
