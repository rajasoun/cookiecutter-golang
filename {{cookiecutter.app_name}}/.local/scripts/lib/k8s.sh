...
# Function: wait for cert-manager CRDs
function wait_for_cert_manager_crds() {
    ...
    pretty_print "${YELLOW}Waiting for cert-manager CRDs to be available${NC}\n"
    
    for crd in "${crds[@]}"; do 
        kubectl wait --for=condition=established --timeout=60s crd/"$crd" || { 
            echo "CRD $crd not available"
            exit 1
        } 
    done
    
    pretty_print "${GREEN}cert-manager CRDs are available${NC}\n"
}

# Function: wait for cert-manager CRDs
function wait_for_open_telemetry_crds() {
    local crds=(
        "instrumentations.opentelemetry.io"
        "opentelemetrycollectors.opentelemetry.io"
    )
    
    pretty_print "${YELLOW}Waiting for opentelemetry CRDs to be available${NC}\n"
    ...
    pretty_print "${GREEN}opentelemetry CRDs are available${NC}\n"
}