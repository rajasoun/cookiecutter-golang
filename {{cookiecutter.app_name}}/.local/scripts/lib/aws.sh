...
readonly DEFAULT_NAMESPACE="aws-otel-collector"
...
function ensure_namespace_exists() {
    ...
    if ! kubectl get namespace "$namespace" &> /dev/null; then
        ...
        if [[ $? -ne 0 ]]; then
            pretty_print "\n${RED}Failed to create namespace: $namespace${NC}\n"
            exit 1
        fi
    fi
}
...
function create_secrets_in_cluster_from_aws_credential_file() {
    ...
    check_secret_exist_in_cluster "$secret_name" "$namespace" && return 1
    kubectl create secret generic "$secret_name" --from-file="$aws_credentials_file" -n "$namespace"
    pretty_print "\n${BLUE}Secrets Created in Cluster from $aws_credentials_file in $namespace ${NC}\n"
}

# Deletes the secret from the cluster
...
function check_aws_token_expiration_time() {
    ...
    local aws_token_expiration_time=$(kubectl get secret $secret_name -n $namespace -o jsonpath='{.data.credentials}' | base64 --decode | aws configure get x_security_token_expires --profile "$aws_profile")
...
function view_aws_credentials_from_cluster() {
    ...
    check_secret_exist_in_cluster $secret_name || return 1
    kubectl get secret $secret_name -n $namespace -o jsonpath='{.data.credentials}' | base64 --decode
}
...
function check_secret_exist_in_cluster() {
    ...
    secret_output=$(kubectl get secret "$secret_name" -n "$namespace" 2>/dev/null)
